...
type
  ...
  public
    class function getFtpPath( Path : string ) : string;
    class function getFtpName( Path : string ): string;
    class function getFtpDir( Path : string ): string;
  public
  ...
  MySizeUtil = class
  public
    class function getStr(FileSize: Int64): string;
  end;

  MyPercentage = class
  public
    class function getStr( Position, FileSize : Int64 ): string;
    class function getInt( Position, FileSize : Int64 ): Integer;
  end;

  MyRemainTime = class
  public
    class function getStr( Second : Int64 ): string;
  end;

   MySpeed = class
  public
    class function getSpeedStr(Speed: Integer): string;
  end;
  ...
  public
    ...
    class function getNewFolder : Integer;
  public
...
begin
  if FileExists( Path ) then
    Result := getPathIcon( Path, True )
  else
  if DirectoryExists( Path ) then
    Result := getPathIcon( Path, False )
  else
    Result := getPathIcon( Path, ExtractFileExt( Path ) <> '' );
end;
...
class function My16IconUtil.getNewFolder: Integer;
begin
  Result := getIcon( LocalIcon_NewFolder );
end;
...
class function MyFilePath.getFtpDir(Path: string): string;
begin
  Result := TPath.GetDirectoryName( Path );
end;

class function MyFilePath.getFtpName(Path: string): string;
begin
  Result := TPath.GetFileName( Path );
end;

class function MyFilePath.getFtpPath(Path: string): string;
begin
  if Path = '' then
    Result := ''
  else
...
class function MySizeUtil.getStr(FileSize: Int64): string;
const
...
class function MyPercentage.getInt(Position, FileSize: Int64): Integer;
begin
  if FileSize > 0 then
  begin
...
class function MyPercentage.getStr(Position, FileSize: Int64): string;
begin
  Result := IntToStr(getInt(Position,FileSize)) + Sign_Percentage;
end;
...
begin
  Result := MySizeUtil.getStr(Speed) + Sign_Speed;
end;

{ MyRemainTime }

class function MyRemainTime.getStr(Second: Int64): string;
begin
  if Second > 3600 then
    Result := IntToStr( Second div 3600 ) + '小时'
  else
  if Second > 60 then
    Result := IntToStr( Second div 60 ) + '分钟'
  else
